...
public class Choose_stack {
  public void choose(Scanner in,stack_control A_control, validation valid, char color,int size,Character id) {
    String input = new String();
    input_converter converter = new input_converter("A1V");
    if (id != 'C') {
    ...
    while (!valid.validStack(converter.x, converter.y, size, converter.direction, A_control.stackArray)) {
      if (id != 'C') {
      ...
      if (!valid.validStack(converter.x, converter.y, size, converter.direction, A_control.stackArray)) {
        continue;
      }
    }
    //Input is valid, add the stack into the stack array
    A_control.addStack(converter.x, converter.y, color, size, converter.direction);
  }
}
